        ...
        <div class="body-content h-100">
            <div class="row g-0 h-100">
                <div class="sidebar col-lg-2 collapse d-lg-block" id="button-primary">
                    ...
                    <a href="{{ route('category.index') }}" class="{{ (request()->is('categories*') ? 'active' : ''); }}">Kategori</a>
                    <a href="{{ route('users.index') }}" class="{{ (request()->is('users*') ? 'active' : '') }}">User</a>
                    <a href="{{ route('rent_logs.index') }}" class="{{ (request()->is('rent-logs*') ? 'active' : '') }}">Peminjaman</a>
                    <a href="{{ route('bukulist.index') }}" @if(request()->route()->uri == 'buku-list') class="active"
                        @endif>Buku</a>
                    <a href="{{ route('book-rent.index') }}" class="{{ (request()->is('book-rent*') ? 'active' : '') }}">Peminjaman Buku</a>
